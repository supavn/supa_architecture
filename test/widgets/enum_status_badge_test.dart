...
import 'package:supa_architecture/widgets/molecules/enum_status_badge.dart';
...
void main() {
  testWidgets(
      'EnumStatusBadge falls back bg/border to color token when bg missing',
      (tester) async {
    final model = EnumModel()
      ..name.rawValue = 'Info'
      ..color.rawValue = 'information'
      ..backgroundColor.rawValue = null;

    await tester.pumpWidget(
      MaterialApp(
        theme: buildTheme(),
        home: Scaffold(
          body: EnumStatusBadge(status: model),
        ),
      ),
    );
